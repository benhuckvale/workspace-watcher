...
describe('workspaceDirectoryImplied', () => {
  ...
  it('should return parent directory for info.json when file exists', () => {
    ...
    const impliedDirectory = workspaceDirectoryImplied(filePath, workspacePatterns);
  ...
  it('should return parent directory for logs/*.log when file exists', () => {
    ...
    expect(workspaceDirectoryImplied(filePath, workspacePatterns)).toBe('path/to');
  });

  it('should return undefined when file does not exist', () => {
    const filePath = 'path/to/nonexistent/file.txt';
    const workspacePatterns = new WorkspacePatterns(['info.json']);
    expect(workspaceDirectoryImplied(filePath, workspacePatterns)).toBeUndefined();
  });
...
describe('isWorkspace', () => {
  test('should return true when all workspace patterns exist', () => {
    ...
    fastglob.sync.mockImplementation((pattern: string) => {
    ...
    expect(fastglob.sync).toHaveBeenCalledWith('info.json', { cwd: '/path/to/workspace' });
    expect(fastglob.sync).toHaveBeenCalledWith('logs/*.log', { cwd: '/path/to/workspace' });
